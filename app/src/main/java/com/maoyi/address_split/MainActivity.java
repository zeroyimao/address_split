package com.maoyi.address_split;
...
public class MainActivity extends AppCompatActivity {
    ...
    private Boolean BooleanProvince,BooleanCity,BooleanArea;
    ...
    private SmartObj smartAddressParse(String text) {
        ...
        Log.i("AddressMsg", "data size===" + data.size());
        text = stripscript(text);
        Log.i("AddressMsg", "text===" + text);
        String[] textArr = text.split("\\s+");
        List<String> dataList = new ArrayList<>(Arrays.asList(textArr));
        while (dataList.remove(null));
        while (dataList.remove(""));
        while (dataList.remove(" "));
        if (dataList != null && !dataList.isEmpty()) {
            Log.i("AddressMsg", "textArr===" + dataList.size() + "|" + dataList);
            ...
            if (dataList.size() > 3) {
                ...
                for (int i = 0; i < dataList.size(); i++) {
                    ...
                    Log.i("AddressMsg", "初次容错过滤i====" + i + " | " + str);
                    //电话匹配
                    if (ConverUtils.isPhoneNumber(str)) {
                        phoneIndex = i;
                        Log.i("AddressMsg", "识别到号码位置===" + phoneIndex + "|" + str);
                        break;
                    }
                }
                ...
                if (phoneIndex == 0) { //开头位置
                    ...
                    for (int i = 1; i < dataList.size(); i++) {
                        ...
                        if (str.length() >= 2 && proviceIndex == -1) {
                            ...
                            Log.i("AddressMsg", "手机号在开头位置str==>" + i + "|str:" + str + " |matchStr:" + matchStr);
                            for (City city : data) {
                ...
                if (address.length() >= 4) {
                    ...
                    for (int endIndex = 1; endIndex < address.length(); endIndex++) {
                        if (endIndex + 1 <= address.length()) {
                            ...
                            Log.i("AddressMsg", "省匹配==" + endIndex + "| " + matchAddress);
                            for (City city : data) {
                                if (city.getAreaName().indexOf(matchAddress) != -1) { //第一次出现的位置 若没有则为-1
                                    ...
                                    Log.i("AddressMsg", "初略省匹配===" + province);
                                }
                            }
                        }
                    }
                    Log.i("AddressMsg", "累积省匹配成功的数量==" + matchProvince.size());
                    if (0==matchProvince.size()){
                        BooleanProvince =false;
                    }else {
                        BooleanProvince =true;
                    }
                    //统计筛选初略统计出的省份
                    ...
                    if (!matchProvince.isEmpty()) {
                        ...
                        Log.i("AddressMsg", "【province】===" + province + "|" + provinceCode);
                        ...
                        Log.i("AddressMsg", "address去除省以后====" + address);
                    }
                    ...
                    for (int endIndex = 1; endIndex < address.length(); endIndex++) {
                        if (endIndex + 1 <= address.length()) {
                            matchAddress = address.subSequence(0, endIndex + 1).toString();
                            Log.i("AddressMsg", "市匹配==" + smartProvice + "|" + endIndex + "| " + matchAddress);
                                        ...
                                        if (smartProvice.equals(item.getAreaName())) {
                                            for (City.CitiesBean.CountiesBean res : item.getCounties()) { //区
                                                if (!TextUtils.isEmpty(matchAddress) && res.getAreaName().indexOf(matchAddress) != -1) {
                                                    Log.i("AddressMsg", "getAreaName==>" + res.getAreaName() + "|" + matchAddress);
                    ...
                    Log.i("AddressMsg", "累积市匹配成功的数量==" + matchCity.size());
                    if (0==matchCity.size()){
                        BooleanCity =false;
                    }else {
                        BooleanCity =true;
                    ...
                    if (!matchCity.isEmpty()) {
                        ...
                        Log.i("AddressMsg", "【city】===" + smartObj.getCity() + "|" + smartObj.getCityCode());

                        address = address.replaceFirst(rsCity.getMatchValue(), "");
                        Log.i("AddressMsg", "address去除市以后====" + address);
                    }
                    ...
                    for (int endIndex = 1; endIndex < address.length(); endIndex++) {
                        if (endIndex + 1 <= address.length()) {
                            matchAddress = address.subSequence(0, endIndex + 1).toString();
                            Log.i("AddressMsg", "区匹配==" + endIndex + "| " + matchAddress);
                            for (City city : data) { //省
                                            ...
                                            if (res.getAreaName().indexOf(matchAddress) != -1) {  //匹配到区了===
                                                ...
                                                if (ConverUtils.isEmpty(smartObj.getProvince())) { //省为空
                                                    if (ConverUtils.isEmpty(smartObj.getCityCode())) {//市为空
                                                        ...
                                                        Log.i("AddressMsg", "areaId===" + areaId);
...
//                            }
                    ...
                    Log.i("AddressMsg", "累积市区配成功的数量==" + matchCounty.size());

                    if (0==matchCounty.size()){
                        BooleanArea =false;
                    }else {
                        BooleanArea =true;
                    }
                    ...
                    if (!matchCounty.isEmpty()) {
                        ...
                        Log.i("AddressMsg", "【area】===" + smartObj.getArea() + "|" + smartObj.getArea());
                        address = address.replaceFirst(rsCity.getMatchValue(), "");
                        Log.i("AddressMsg", "address去除区以后====" + address);


                        smartObj.setAddr(address);
                        Log.i("AddressMsg", "addr=====" + address);
                        continue;
                    }else {
                        smartObj.setAddr(allAdress);
                        Log.i("AddressMsg", "累积市区配成功的数量==matchAddress" + allAdress);
                    }
                }

                //姓名查找
                if (ConverUtils.isEmpty(smartObj.getProvince())&address.length()<5) {

                    smartObj.setName(address);
                    Log.i("AddressMsg", "name====zoule" + address);
                }
            }
        }
        return smartObj;
    ...
    private String stripscript(String s) {
        Log.i("AddressMsg", "s 处理前===" + s);
        Pattern p = Pattern.compile("(\\d{3})\\-(\\d{4})\\-(\\d{4})$*");
        Matcher m = p.matcher(s);
        s = m.replaceAll("$1$2$3");
        Log.i("AddressMsg", "s 处理中===" + s);

        String pattern = "[`~!@#$^&*()=|{}':;',\\[\\].<>/?~！@#￥……&*（）——电话|姓名{}地址【】手机号码‘；：”“’。，、？-]";
        String rs = "";
...
//        mt.lookingAt();
        ...
        while(mt.find()){
            pos = mt.start();
            Log.i("AddressMsg","findPhonePostion==" + mt.group(0) + "|||" + mt.start());
        }
        return pos;
    }
